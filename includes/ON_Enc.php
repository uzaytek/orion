...
/**
 * Encription object for ids
 */
class ON_Enc extends encryption_class {
  ...
  public function __construct() {
    parent::encryption_class();
    $this->key = 'mostsecretkey'; // key must not have special chars (quotes,-,etc)
    $this->key_len = 32;
  }
   ...
   *
   * @param mixed $val id or string which will be encrypt
   * @return string encrypted value
   */
  public function encrypt($val) {
    return parent::encrypt($this->key, $val, $this->key_len);
  }
  
  /**
   * Decrypt parameter value
   *
   * @param mixed $val id or string which will be decrypt
   * @return string decrypted value
   */
  public function decrypt($val) {
    $ret = parent::decrypt($this->key, $val);
    if (!$ret) {
      $log = new ON_Log;
      $log->insert(implode(":", $this->errors));
      return false;
    } else {
      return $ret;
    }
...
class encryption_class {
    ...
    function encryption_class ()
    {
        ...
        $this->scramble1 = 'QWERTYUIOPASDFGHJKLZXCVBNM_qwertyuiopasdfghjklzxcvbnm0123456789';
        $this->scramble2 = 'zaqxswcdevfrbgtnhymjukilop_ZAQXSWCDEVFRBGTNHYMJUKILOP8527419630';
    ...
    // encrypt string into a garbled form
    {
        ...
        //n("<h1>1--".strlen($source)."</h1>");
        $source = str_pad($source, $sourcelen,'_');
        //n("<h1>2--".strlen($source)."</h1>");

        $target = null;
        $factor2 = 0;
        //n('<h1>3--'.strlen($source).'</h1>');
        $strlen = strlen($source);
        //n('<h1>4--'.strlen($source).'</h1>');
