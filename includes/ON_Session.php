 ...
 *
 */ 
class ON_Session extends ON_Dao {
  ...
  function __construct($name=null) {
    ...
    $this->sessionName = (defined('ON_SESSION_NAME')) ? ON_SESSION_NAME : $name;
    if(!headers_sent()) {
      ob_start();  
    ...
    try {
      ...
      if ($adminname != '' && $adminpasswd != '') {
        $result = self::$db->query('SELECT * FROM '.DB_TBL_ADMINS . 
                                   ' WHERE adminname = '.$this->quote(ON_Filter($adminname)).
                                   ' AND adminpasswd = ' .$this->quote(ON_Filter($adminpasswd)));
        $row = $result->fetch();
      }
      if (!is_array($row)) {// error
        ...
        if (!$this->ifset('sid')) {
          ...
          self::$db->query('INSERT INTO '.DB_TBL_SESSIONS.' SET sid='.$this->quote($sid).' ,ip=' . 
                           $this->quote(ON_Filter($_SERVER['REMOTE_ADDR'])).' , time=NOW()');
        }
        return true;
      }
    } catch (PDOException $e) {
  ...
  public function controlSession() {
    try {
      ...
      $query = 'SELECT COUNT(*) AS NUMROWS FROM ' . DB_TBL_SESSIONS . ' AS s,' . DB_TBL_ADMINS . ' AS u' .
        ' WHERE s.sid='.$this->quote(ON_Filter($this->get('sid'))) .
        ' AND s.sid=u.adminsid AND s.ip = '.$this->quote(ON_Filter($_SERVER["REMOTE_ADDR"]));
      $result = self::$db->query($query);
      $numrows = $result->fetchColumn();
      if ($numrows > 0) {
        self::$db->query('UPDATE ' . DB_TBL_SESSIONS.' SET time=NOW() WHERE sid="'.ON_Filter($this->get('sid')).'"');
        return true;
      } else {
        return false;
      }
  ...
  public function instance() {
    if(!ON_Session::$instance) {
      ON_Session::$instance = new ON_Session;
    }
    return ON_Session::$instance;
  }
  ...
  function set($k, $v, $bOverWrite=true) {
    if ($this->ifset($k) && $bOverWrite == false) {
      $_SESSION[$this->sessionName][$k] .= ON_Filter($v);
    } else {
      $_SESSION[$this->sessionName][$k] = ON_Filter($v);
    }
  }
