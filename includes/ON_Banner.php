 ...
 * 
 */  
class ON_Banner extends ON_Dao
{
  ...
  public function fillForm() {
    ...
    $this->form->addRule('bannerfile', 
                         sprintf(_('Maximum file size is: %d KB'), fmtKB($GLOBALS['_conf']['HTML_MAX_FILE_SIZE']['banner'])), 
                         'maxfilesize', $GLOBALS['_conf']['HTML_MAX_FILE_SIZE']['banner']);
    $this->form->addRule('bannerfile', _('File extension required in types:'),
                         'mimetype', $GLOBALS['_conf']['HTML_PERMIT_TYPES']['banner']);
    // Tell well-behaved browsers not to allow upload of a file larger than max file size
    $this->form->setMaxFileSize($GLOBALS['_conf']['HTML_MAX_FILE_SIZE']['banner']);
  ...
  public function setValues(&$values, $path) {
    ...
    if ($file->isUploadedFile()) {
      ...
      if ($file->moveUploadedFile($path, $newname)) {
        ...
        // move upload failed
        ON_Say::add(fmtError(_('Upload error: banner upload failed')));
        return false;
      }
    } // end is uploaded file
    $this->location = ON_Filter($values['location']);
    $this->url      = ON_Filter($values['url']);    
    return true;
  }
